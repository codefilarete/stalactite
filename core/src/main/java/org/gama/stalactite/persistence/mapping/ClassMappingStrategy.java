...
public class ClassMappingStrategy<T> implements IMappingStrategy<T> {
	 ...
	 * @return columns aff all mapping strategies without auto generated keys
	 */
	...
	private void fillInsertableColumns() {
		...
		insertableColumns.addAll(defaultMappingStrategy.getColumns());
		...
		for (Column column : getKeys()) {
			if (column.isAutoGenerated()) {
				insertableColumns.remove(column);
			}
		}
	}
