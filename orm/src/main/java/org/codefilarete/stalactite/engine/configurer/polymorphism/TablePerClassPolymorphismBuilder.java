...
class TablePerClassPolymorphismBuilder<C, I, T extends Table<T>> extends AbstractPolymorphicPersisterBuilder<C, I, T> {
	 ...
	 */
	public static <C, T extends Table<T>, A extends PropertyAccessPoint<C, ?>> Map<A, Column<T, ?>>
	projectColumns(Map<? extends A, ? extends Column<?, ?>> propertyToColumn,
				   T targetTable,
				   BiFunction<? super A, Column<?, ?>, String> columnNameSupplier) {
		Map<A, Column<T, ?>> localResult = new HashMap<>();
		propertyToColumn.forEach((accessor, column) -> {
			Column<T, ?> projectedColumn = targetTable.addColumn(columnNameSupplier.apply(accessor, column), column.getJavaType(), column.getSize(), column.isNullable());
	...
	buildSubclassPersister(Dialect dialect,
		...
		Map<ReadWritePropertyAccessPoint<D, ?>, Column<SUBTABLE, ?>> subEntityPropertiesMapping = embeddableMapping.getMapping();
		Map<PropertyMutator<D, ?>, Column<SUBTABLE, ?>> subEntityReadonlyPropertiesMapping = embeddableMapping.getReadonlyMapping();
		...
		Map<PropertyAccessPoint<C, ?>, Column<SUBTABLE, ?>> projectedMainMapping = projectColumns(mainMapping, subTable, (accessor, c) -> c.getName());
		subEntityPropertiesMapping.putAll((Map) projectedMainMapping);
		Map<PropertyAccessPoint<C, ?>, Column<SUBTABLE, ?>> projectedMainReadonlyMapping = projectColumns(mainReadonlyMapping, subTable, (accessor, c) -> c.getName());
